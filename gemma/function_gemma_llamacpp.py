...
def get_room_temperature() -> Dict[str, Any]:
    """Retourne la température et l'humidité de la pièce (valeurs fictives)."""
    def fn_return():
        return "@ask_temperature"
    
   # message = Message.sensor("GEMMA", SensorId.TEMPERATURE, fn_return(), "ALL")
    # ws.send(message, "ALL")
    
    return fn_return()

def turn_on_light(led_id: int) -> Dict[str, Any]:
    """Allume une LED spécifique (simulation)."""
    return {
        "state": "on",
        "led_id": led_id,
        "sensor_id": SensorId.LED
    }
...
TOOLS_DECL: List[Dict[str, Any]] = [
    ...
    },
    {
        ...
        "function": {
            "name": "turn_on_light",
            "description": "Allumer une LED spécifique.",
            "parameters": {
                "type": "object",
                "properties": {
                    "led_id": {"type": "integer"},
...
TOOL_DECLARATIONS_TEXT = [
    ...
    {
        "name": "turn_on_light",
        "description": "Turn on a specific LED light.",
        "parameters": {
            "type": "object",
            "properties": {
                "led_id": {"type": "integer"},
...
Example: If user says "Turn on the LED number 3" or "Allume la led 3", respond with:
{"name": "turn_on_light", "parameters": {"led_id": 3}}
